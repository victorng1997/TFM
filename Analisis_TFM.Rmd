...
```{r include=FALSE}
# Se crea la función que se utilizará para hacer los violin plot.
...
## Se cargan los paquetes que se vayan a necesitar
if (!(require(knitr))){
...
# Seleccionamos las variables categóricas para luego trabajar con ellas.
...
# Creamos un vector con todas las variables
...
# Guardamos la base de datos original en otro objeto
data_full <- data
```
...
# Creamos una tabla descriptiva de las variables a usar
covariates <- c("ORSEX","SEXONOFF","CONDUCTA", "ABAND","PATRT","PAECC","TIHIJ","PAENF","PAALC","PASUS","PASUS01","PASUS02","PASUS03","PASUS04","PASUS05","PASUS06","PACAU","studies","employed","tabaco","PAORG", "NSTOTAL", "HATOTAL", "RDTOTAL", "PSTOTAL", "SDTOTAL", "CTOTAL", "STTOTAL", "VALID", "Inicio", "evoluc", "PAAPRO", "PAANOS", "PAHIJ", "PAEDA")
...
set.seed(123) # Establecems una semilla para obtener los mismos valores en cada iteración

# Creamos vectores con el numero de columna para los distintos tipos de variables
...
data[,covariates[indices_numeric]] <- scale(data[,covariates[indices_numeric]]) # Escalamos las variables numéricas del data fram


results <- lapply(2:6, function(k) kproto(data[,covariates], k = k)) # Calculamos para cada k de 2 a 6 los valores de cluster del k-prototypes

# Calculamos la distancia de huang, la cual se usa en el k-prototypes
...
# calculamos los valores del MCA para las variables categóricas del modo de datos mixtos
...
# Calculamos el hopkins statistica para las variables numéricas tanto de las usadas en los métodos mixtos como de las de TCI-R
...
# Calculamos para cada resultado de k de k-prototypes los distintos estadísticos de performance
...
# Para cada k se dibujan los valores de PCA con los cluster obtenidos con k-prototypes
...
gower_dist <- daisy(data[,covariates],"gower") # Se calcula la distancia de gower para usar en los proximos estadísticos ya que kamila no usa una distancia como tal.


# Se calcula para k de 2 a 6 los resultados del metodo de cluster kamila
...
# Calculamos para cada resultado de k de kamila los distintos estadísticos de performance usando la distancia de gower
...
# Para cada k se dibujan los valores de PCA con los cluster obtenidos con kamila
p1 <- fviz_cluster(
...
```{r message=FALSE, warning=FALSE, include=FALSE}
# Se calcula para cada k de 2 a 6 los resultados de PAM con ditancia de gower
...
# Calculamos para cada resultado de k de k-medoids los distintos estadísticos de performance usando la distancia de gower
...
# Para cada k se dibujan los valores de PCA con los cluster obtenidos con k-medoids
...
# El método de cluster que mejor ha funcionado se usa para crear una tabla descriptiva de distintas variables para ver si hay diferencias entre los clusters.
...
```{r echo=FALSE, fig.height=8, fig.width=16, message=FALSE, warning=FALSE}
# Se ajusta un modelo logístico para ver si hay diferencias significativas (SIN CONTAR EL TIEMPO) entre el dropout de los distintos clusters.
...
# Se ajusta un modelo supervivencia para ver si hay diferencias significativas entre el dropout de los distintos clusters.
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
# Se cambia ligeramente la funcion de los violin plot para ajustarse a 3 grupos
...
# Se vuelven a ajustar las primeras variables y la semilla 
...
# Se ajusta un modelo k means para cada k de 2 a 6 y se muestran los resultados del PCA ajustando por los clusters
...
# Calculamos para cada resultado de k de k means los distintos estadísticos de performance usando la distancia euclidea
...
# Se calcula los valores de PCA de las variables de TCI-R
pca_res <- prcomp(scale(data[,covariates_TCI]), scale. = TRUE)
pca_data <- data.frame(pca_res$x[, 1:2])

# Se calcula la ditancia euclidea
dist_matrix <- dist(scale(data[,covariates_TCI]), method = "euclidean")

# Se calcula mediante el clustering jerarquico con el método de ward los clusters para cada k de 2 a 6
hclust_res <- hclust(dist_matrix, method = "ward.D2")

clusters_hc_1 <- cutree(hclust_res, k = 2)  
...
# Calculamos para cada resultado de k de clustering jerarquico los distintos estadísticos de performance usando la distancia euclidea
...
# Se calcula mediante gaussian mixture models los clusters para cada k de 2 a 6
...
# Calculamos para cada resultado de k de gaussian mixture models los distintos estadísticos de performance usando la distancia euclidea
silhouette_index <- NA
avg_silhouette <- vector()
davies_bouldin <- vector()
dunn_index <- vector()
...
p1 <- fviz_cluster(
  ...
  theme_minimal() +
  labs(title = "Gaussian Mixture models Representado en PCA")
...
# El método de cluster que mejor ha funcionado (hierarchical clustering) se usa para crear una tabla descriptiva de distintas variables para ver si hay diferencias entre los clusters.
